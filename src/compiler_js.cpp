...
public:
    explicit TypeChoice(const ast::TypeChoice *tc): Type(tc), tc(tc) {}
...
public:
    ...
    virtual void generate(Context &context) const override {
        if (pv->name == "io$stderr") {
            context.out << "null";
        } else if (pv->name == "sys$args") {
            context.out << "process.argv";
        } else {
            context.out << pv->name;
        }
    }
...
public:
    ...
    virtual void generate(Context &context) const override {
        context.out << "throw new neon.NeonException(" << quoted(rs->exception->name) << ", ";
...
public:
    ...
    virtual void generate() const {
        ...
        support->writeOutput(path + program->module_name + ".js", src);
    }
};
...
void compile_js(CompilerSupport *support, const ast::Program *p, std::string /*output*/, std::map<std::string, std::string> /*options*/)
{
    js::g_type_cache.clear();
    js::g_variable_cache.clear();
    js::g_expression_cache.clear();
    js::g_statement_cache.clear();

    js::Program *ct = new js::Program(support, p);
    ct->generate();
}
